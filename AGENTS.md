...
Image: `quay.io/<org>/oct-network-bond:1.1.0-ocp4.22` (`<semver>-ocp<major.minor>`). Git: tags `v1.x.x`; `main` / optional `ocp-4.22` when PF/API differ. PatternFly 6 on 4.22.
...
Catalog PR against `oct-storefront` `catalog/community.yaml` (`source: community`, `category: network`, `spec.versions[]` with `version` + `openshift` + combined `image`). `spec.href` must stay `/community-tools/network/bond` unless `console-extensions.json` changes.
